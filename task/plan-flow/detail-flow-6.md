...
* **Mục tiêu**: Xây dựng cơ chế tự động hủy đơn hàng và hoàn trả tồn kho tạm giữ (Reserved Stock) khi khách hàng không hoàn tất thanh toán trong khung thời gian quy định (Time-To-Live - TTL: **cố định 2 phút / 120 giây** nhằm phục vụ kiểm thử và demo nhanh cho đồ án). Ngăn chặn hành vi "giữ chỗ ảo" làm đóng băng kho hàng, đảm bảo tính quay vòng vốn cho Seller và tạo cơ hội mua hàng cho các độc giả khác.
* **Các bên tham gia (Actors)**:
  ...
  2. **Hệ thống Lập lịch & Xử lý nền (Background Workers & Schedulers)**:
     ...
     * `notification-service`: Gửi thông báo nhắc nhở trước khi hết hạn (ở phút thứ 1 / 60s) và thông báo hủy đơn khi hết hạn (ở phút thứ 2 / 120s).
...
[00:00] ─── TẠO ĐƠN HÀNG (PENDING_PAYMENT)
  ...
  │          • Tạo PayOS VietQR Code (Thời hạn: 02:00 / 120 giây)
  │
[01:00] ─── CẢNH BÁO NHẮC NHỞ THANH TOÁN (Sau 1 phút / Còn 60s)
  │          • Gửi Web Push / Realtime Toast: "Đơn hàng của bạn sẽ hết hạn sau 60 giây!"
  │          • Đếm ngược chuyển màu ĐỎ nhấp nháy trên trang thanh toán
  │
[02:00] ─── TIMEOUT - TỰ ĐỘNG THU HỒI KHO (Hết 2 phút / 120 giây)
  │          • Hủy đơn hàng: Trạng thái chuyển sang CANCELLED (Lý do: EXPIRED)
...
* **Phương thức PayOS VietQR / VNPAY / MoMo**: Thời gian giữ kho là **2 phút (120 giây)** kể từ thời điểm bấm "Đặt Hàng".
* **Mốc cảnh báo khẩn cấp (Warning Milestone)**: **Sau 1 phút (60 giây)** (khi thời gian còn lại $\le 60\text{s}$).
* **Đơn hàng Flash Sale (Sản phẩm Giờ Vàng)**: Thời gian giữ kho rút ngắn còn **1 phút (60 giây)**.
...
| `expires_at` | Timestamp | Bắt buộc nếu là thanh toán online | Thời điểm chính xác đơn hàng hết hạn (`created_at + 2m` / `120s`) |
| `cancel_reason` | String | Nullable | Lý do hủy: `"Hết hạn thanh toán (Auto Timeout 2m / 120s)"` |
...
sequenceDiagram
    ...
    Note over Buyer,OrderSvc: GIAI ĐOẠN 1: TẠO ĐƠN HÀNG VÀ HẸN GIỜ (T = 0s)
    Buyer->>OrderSvc: POST /orders (Phương thức: PayOS VietQR)
    OrderSvc->>InvenSvc: reserveStock(BookId, Qty: 2) -> Khóa 2 cuốn
    OrderSvc->>Queue: Thêm Delayed Job `order_timeout_check` (Delay = 120s, OrderId)
    OrderSvc-->>Web: Trả về QR Code + Thời gian hết hạn (expires_at)
    Web-->>Buyer: Hiển thị đồng hồ đếm ngược 02:00

    Note over Queue,OrderSvc: GIAI ĐOẠN 2: CẢNH BÁO PHÚT THỨ 1 (T = 60s)
    Queue->>OrderSvc: Trigger Job nhắc nhở (Sau 60s)
    OrderSvc->>Web: Bắn Socket / SSE thông báo: "Còn 60 giây để thanh toán đơn hàng!"

    Note over Queue,InvenSvc: GIAI ĐOẠN 3: HẾT HẠN & THU HỒI TỒN KHO (T = 120s)
    Queue->>OrderSvc: Trigger Job `order_timeout_check` (Hết 120s / 2m)
...
* **Bước 1.1: Tạo đơn và tính toán mốc `expires_at`**:
  ...
  * Hệ thống tính toán: `expires_at = NOW() + INTERVAL '2 minutes'`.
...
* **Bước 1.2: Đẩy Delayed Job vào hàng đợi xử lý nền**:
  * `order-service` đẩy 2 job vào BullMQ:
    1. `Job 1 (Reminder)`: Trì hoãn 1 phút (`delay = 60,000ms`).
    2. `Job 2 (AutoCancel)`: Trì hoãn 2 phút (`delay = 120,000ms`).
...
* **Bước 2.1: Đồng hồ đếm ngược trên trang thanh toán (`PaymentWaitingPage.jsx`)**:
  ...
  * Hiển thị dạng `MM:SS` (VD: `01:59` &rarr; `01:58`...).
* **Bước 2.2: Thay đổi trạng thái thị giác theo thời gian**:
  * Từ `02:00` đến `01:01`: Đồng hồ hiển thị màu Xanh lam trung tính.
  * Từ `01:00` đến `00:00`: Đồng hồ chuyển sang màu **Đỏ nhấp nháy** kèm thông điệp khẩn cấp *"Vui lòng quét mã thanh toán trước khi đơn bị hủy tự động!"*.

---

### BƯỚC 3: XỬ LÝ SỰ KIỆN HẾT HẠN THANH TOÁN (EXECUTE TIMEOUT TRANSACTION)

* **Bước 3.1: Bộ xử lý nền (Worker) tiếp nhận sự kiện sau 2 phút (120s)**:
...
* **Bước 3.2: Phân xử an toàn (Idempotency & Race Condition Handling)**:
  * **Trường hợp A (Khách đã trả tiền vào phút 01:45, Webhook đã xác nhận `PAID`)**:
    ...
    * Tiến hành mở Database Transaction thực hiện chuỗi hành động nguyên tử:
      ...
      2. Cập nhật đơn hàng:
         ...
         SET 
             ...
             cancelled_at = NOW(),
             cancel_reason = 'Hết hạn thanh toán (Hệ thống tự động hủy sau 2 phút)'
...
* **Bước 4.3: Ghi vết kiểm toán kho**:
  * Tạo bản ghi `inventory_logs`:
    * `action_type = 'RELEASE'`.
    * `order_id = :orderId`.
    * `note = 'Tự động hoàn trả tồn kho do đơn hàng hết hạn 2m'`.
...
* **Kịch bản Tranh chấp**: Khách hàng chuyển khoản đúng giây thứ **01:59**. Webhook PayOS gửi về server chậm 3 giây (đến lúc 02:02, sau khi hệ thống đã chạy Timeout hủy đơn và nhả kho).
...
|:---:|---|---|---|:---:|
| **TC_TO_01** | Hết 2 phút không thanh toán | Đơn hàng 2 cuốn sách, không quét mã QR | Sau đúng 2 phút (120s), đơn chuyển `CANCELLED`, tồn kho khả dụng tăng lại +2. | **PASS** |
| **TC_TO_02** | Khách thanh toán ở phút 01:30 | Quét QR và chuyển tiền thành công lúc 01:30 | Đơn chuyển sang `PAID`, Job 2m khi chạy đến sẽ bỏ qua, không hủy đơn. | **PASS** |
| **TC_TO_03** | Khách chủ động bấm "Hủy Đơn" | Bấm nút "Hủy đơn hàng" ở giây thứ 30 | Đơn hủy ngay lập tức, kho được nhả ngay ở giây thứ 30 không cần chờ hết 2m. | **PASS** |
| **TC_TO_04** | Nhắc nhở ở phút thứ 1 | Đơn tạo được 1 phút (60s) chưa thanh toán | Hệ thống gửi thông báo nhắc nhở "Còn 60 giây", đồng hồ đổi màu đỏ. | **PASS** |
| **TC_TO_05** | **Tranh chấp Webhook đến trễ (Edge Case)** | Tiền vào lúc 02:01 (sau khi đã hủy đơn). Kho còn hàng. | Hệ thống tự động kích hoạt khôi phục đơn sang `PAID` và khóa lại kho an toàn. | **PASS** |
| **TC_TO_06** | Tranh chấp khi kho đã hết hàng | Tiền vào lúc 02:01, kho đã hết sách. | Kích hoạt luồng hoàn tiền `AUTO_REFUND` 100% cho khách hàng, gửi email giải thích. | **PASS** |

---

## VII. ĐIỀU KIỆN NGHIỆM THU HOÀN TẤT (DEFINITION OF DONE)

1. ✅ Thiết lập chính xác cơ chế đếm ngược TTL 2 phút (120 giây) thông qua BullMQ Delayed Jobs và Redis.
...
4. ✅ Giao diện người mua hiển thị đồng hồ đếm ngược sinh động (02:00) và thông báo rõ ràng khi đơn bị hủy.
