...
Returns the authenticated buyer's immutable order history. The response is the same order-level status and seller summary used by `/orders/:id/tracking`, including timeline entries generated by payment, seller and shipping events.

```http
GET /api/v1/orders/order-uuid/history
Authorization: Bearer <access_token>
```

The order must belong to the authenticated buyer; otherwise the API returns `404`.
