...
export default function PdfCanvasViewer({
  ...
  const overlayRef1 = useRef<HTMLCanvasElement>(null);
  const overlayRef2 = useRef<HTMLCanvasElement>(null);
  ...
  const redrawHighlights = useCallback((pageNum: number, overlayCanvas: HTMLCanvasElement | null, dpr: number) => {
    ...
    pageStrokes.forEach((stroke: any) => {
      if (stroke.type === 'rect') {
  ...
  const renderPage = useCallback(async (pageNum: number, canvas: HTMLCanvasElement | null, overlayCanvas: HTMLCanvasElement | null) => {
  ...
  }, [pdfDoc, zoom, viewMode, redrawHighlights]);
  ...
  const handlePointerDown = (e: React.PointerEvent<HTMLCanvasElement>, pageNum: number, overlayCanvas: HTMLCanvasElement | null) => {
    ...
    if (highlighterMode === 'eraser') {
      const pageStrokes = (highlights[bookId] && highlights[bookId][pageNum]) || [];
      const hitStroke = pageStrokes.find((s: any) => {
        if (s.type === 'rect') {
          return x >= s.x - 4 && x <= s.x + s.width + 4 && y >= s.y - 4 && y <= s.y + s.height + 4;
        }
        return s.points?.some((p: any) => Math.hypot(p.x - x, p.y - y) <= (s.size || 20) + 10);
      });
      if (hitStroke) {
  ...
  const handlePointerMove = (e: React.PointerEvent<HTMLCanvasElement>, pageNum: number, overlayCanvas: HTMLCanvasElement | null) => {
  ...
  const handlePointerUp = (e: React.PointerEvent<HTMLCanvasElement>, pageNum: number, overlayCanvas: HTMLCanvasElement | null) => {
      ...
      {!loading && !error && (
        <div className="flex items-start justify-center gap-6 max-w-full my-2">
          ...
          <div className="relative shadow-2xl rounded-sm overflow-hidden bg-white shrink-0">
            ...
            {/* Forensic Watermark DRM Layer (Task 55 / POL-04 DRM-002) */}
            <WatermarkOverlay
              pageNum={currentPage}
              bookTitle={bookTitle}
              bookId={bookId}
