...
export interface PdfReaderHeaderProps {
  bookTitle?: string;
  bookAuthor?: string;
  bookId?: string;
}
...
export default function PdfReaderHeader({
  bookTitle = 'Con Đường Phía Trước',
  bookAuthor = 'Bill Gates',
  bookId = 'con-duong-phia-truoc'
    ...
    <header className="h-14 bg-[#18181b] border-b border-[#27272a] px-4 md:px-6 flex items-center justify-between z-40 shrink-0 text-white select-none">
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        <button
          ...
          className="p-1.5 rounded-lg text-zinc-300 hover:text-white hover:bg-white/10 disabled:opacity-20 disabled:hover:bg-transparent transition-colors flex items-center justify-center cursor-pointer disabled:cursor-not-allowed"
          title="Trang trước (←)"
        >
      ...
      <div className="flex items-center gap-1.5 sm:gap-2">
        ...
        <div className="hidden sm:flex items-center bg-[#27272a] rounded-lg p-0.5 border border-zinc-700">
          <button
            ...
            disabled={zoom <= 50}
            className="p-1 rounded text-zinc-300 hover:text-white hover:bg-white/10 disabled:opacity-20 transition-colors cursor-pointer"
            title="Thu nhỏ (-)"
          >
          ...
          <button
            onClick={() => setZoom(100)}
            className="px-2 py-0.5 text-xs font-mono font-bold text-zinc-200 hover:text-white hover:bg-white/10 rounded transition-colors cursor-pointer"
            title="Đặt lại 100%"
          >
            {zoom}%
          </button>
        ...
        <button
          ...
          className={`hidden md:flex p-2 rounded-lg text-zinc-300 hover:text-white hover:bg-white/10 transition-colors items-center justify-center cursor-pointer ${
        ...
        <button
          ...
          className={`p-2 rounded-lg text-zinc-300 hover:text-white hover:bg-white/10 transition-colors flex items-center justify-center cursor-pointer ${
          ...
          {showThemeMenu && (
            <div className="absolute right-0 mt-2 w-48 bg-[#1f2227] text-white rounded-xl shadow-2xl border border-zinc-700 p-2 z-50 text-xs space-y-1">
              ...
              <button
                ...
                className={`w-full flex items-center gap-2 px-2.5 py-2 rounded-lg transition-all cursor-pointer ${
        ...
        {/* Typography & Appearance Settings (Task 52) */}
        <button
          onClick={() => setIsSettingsOpen(!isSettingsOpen)}
          className={`p-2 rounded-lg transition-colors flex items-center justify-center cursor-pointer ${
            isSettingsOpen ? 'bg-emerald-600 text-white shadow-sm' : 'text-zinc-300 hover:text-white hover:bg-white/10'
          }`}
          title="Tùy chỉnh phông chữ & hiển thị"
        >
        ...
        <button
          ...
          className="p-2 rounded-lg text-zinc-300 hover:text-white hover:bg-white/10 transition-colors hidden sm:flex items-center justify-center cursor-pointer"
