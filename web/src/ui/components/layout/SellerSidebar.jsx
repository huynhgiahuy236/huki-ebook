  ...
  const menuGroups = [
    ...
    },
    {
      ...
      items: [
        ...
        { to: '/seller/chat', icon: 'chat', label: 'Tin Nhắn & Chat', disabled: true },
        ...
        { to: '/seller/product/correction', icon: 'report_problem', label: 'Sản Phẩm Cần Sửa', disabled: true }
      ]
    },
    {
      ...
      items: [
        ...
        { to: '/seller/edge-cases', icon: 'account_tree', label: 'Thư Viện Trạng Thái', disabled: true }
      ]
    },
    {
      title: 'HỒ SƠ ĐỐI TÁC',
            ...
            <div className="space-y-1">
              {group.items.map((item, iIdx) => (
                <NavLink
                  ...
                  title={item.disabled ? 'Tạm khóa — ngoài happy case hiện tại' : (isCollapsed ? item.label : undefined)}
                  onClick={(event) => {
                    if (item.disabled) {
                  ...
                  className={({ isActive }) =>
                    `flex items-center rounded-xl text-xs font-semibold transition-all relative group ${
                      ...
                      ? 'text-on-surface-variant opacity-45 cursor-not-allowed'
                      : isActive
                  ...
                  {({ isActive }) => (
                    <>
                      <div className={`flex items-center ${isCollapsed ? 'justify-center' : 'gap-2.5'}`}>
                        ...
                        {!isCollapsed && item.disabled && <span className="text-[9px] uppercase">Tạm khóa</span>}
                      </div>
