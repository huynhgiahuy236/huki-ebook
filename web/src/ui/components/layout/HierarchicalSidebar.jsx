...
export default function HierarchicalSidebar({ isCollapsed, setIsCollapsed, isMobileOpen, setIsMobileOpen }) {
      ...
      items: [
        ...
        },
        {
          ...
          subItems: [
            ...
            { title: 'Sản Phẩm Cần Sửa Lỗi', to: '/seller/product/correction', icon: 'report_problem', disabled: true }
          ]
        },
        {
          id: 'seller-edge',
  ...
  const miniRailItems = [
    ...
    { id: 'mini-community', to: '/community', icon: 'forum', title: 'Mạng Xã Hội Độc Giả', disabled: true },
    { id: 'mini-chat', to: '/chat', icon: 'chat', title: 'Tin Nhắn & Chat Messenger', disabled: true },
    ...(isLoggedIn ? [{ id: 'mini-library', to: '/library', icon: 'auto_stories', title: 'Tủ Sách & Tiến Độ Đọc', disabled: true }] : []),
    ...(isLoggedIn ? [{ id: 'mini-reader', to: '/reader', icon: 'chrome_reader_mode', title: 'Trình Đọc Ebook Web', disabled: true }] : []),
                ...
                <div key={item.id} className="relative group w-full flex justify-center">
                  <NavLink
                    ...
                    className={`
                      ...
                      ${item.disabled
                        ? 'text-[var(--theme-text-muted,#556963)] opacity-35 cursor-not-allowed'
                        : isActive
                  ...
                  <div className="absolute left-[54px] top-1/2 -translate-y-1/2 z-50 pointer-events-none opacity-0 group-hover:opacity-100 group-hover:translate-x-1 transition-all duration-150">
                    <div className="bg-[var(--theme-primary,#003b2b)] text-white text-xs font-semibold px-3 py-1.5 rounded-lg shadow-xl whitespace-nowrap flex items-center gap-1.5 border border-white/10">
                      <span>{item.title}{item.disabled ? ' · Tạm khóa' : ''}</span>
                      {item.badge && (
              ...
              return (
                <div key={group.id} className="space-y-1">
                  ...
                  <button
                    ...
                    title={group.disabled ? 'Tạm khóa — ngoài happy case hiện tại' : undefined}
                    className={`w-full flex items-center justify-between px-2 py-1.5 text-[10.5px] font-bold tracking-wider text-[var(--theme-text-muted,#8d706b)] rounded-lg transition-colors group ${group.disabled ? 'opacity-40 cursor-not-allowed' : 'hover:text-[var(--theme-primary,#003b2b)] cursor-pointer'}`}
                  >
                    <span className="uppercase truncate text-left">{group.title}{group.disabled ? ' · TẠM KHÓA' : ''}</span>
                          ...
                          <div key={item.id} className="space-y-0.5">
                            ...
                            <div className="flex items-center group/item w-full">
                              <NavLink
                                ...
                                aria-disabled={item.disabled || undefined}
                                tabIndex={item.disabled ? -1 : undefined}
                                title={item.disabled ? 'Tạm khóa — ngoài happy case hiện tại' : undefined}
                                className={({ isActive }) => `
                                  flex-1 min-w-0 flex items-center justify-between px-2.5 py-2 rounded-xl text-[12px] font-semibold transition-all
                                  ${item.disabled
                                    ? 'text-[var(--theme-text-muted,#556963)] opacity-40 cursor-not-allowed'
                                    : isActive
                              ...
                              >
                                <div className="flex items-center gap-1.5 min-w-0 flex-1">
                                  ...
                                  <span className="truncate leading-tight">{item.title}{item.disabled ? ' · Tạm khóa' : ''}</span>
                                </div>
                                ...
                                {item.subItems.map((sub, idx) => {
                                  return (
                                    <Link
                                      ...
                                      title={sub.disabled ? 'Tạm khóa — ngoài happy case hiện tại' : undefined}
                                      className={`flex items-center justify-between gap-1.5 px-2 py-1.5 rounded-lg text-[11.5px] font-medium text-[var(--theme-text-muted,#556963)] transition-colors group ${sub.disabled ? 'opacity-40 cursor-not-allowed' : 'hover:text-[var(--theme-primary,#003b2b)] hover:bg-[var(--theme-secondary-subtle,#f2fbf9)]'}`}
                                    >
                                      <div className="flex items-center gap-1.5 min-w-0 flex-1">
                                        ...
                                        <span className="truncate leading-tight">{sub.title}{sub.disabled ? ' · Tạm khóa' : ''}</span>
                                      </div>
                                      {sub.badge && (
                                        <span
