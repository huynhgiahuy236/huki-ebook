...
export default function EscrowCountdown({
  ...
  const handleDisputeClick = () => {
    ...
    if (onDispute) {
      ...
      navigate(`/returns?orderId=${orderId}&subOrderId=${subOrderId}`);
    }
  };
  ...
  if (frozen) {
    return (
      <div className={`p-4 rounded-2xl bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 text-amber-900 dark:text-amber-200 flex items-center justify-between gap-3 ${className}`}>
        ...
        <button
          type="button"
          onClick={() => navigate(`/returns?orderId=${orderId}&subOrderId=${subOrderId}`)}
          className="px-3 py-1.5 rounded-xl bg-amber-600 text-white text-xs font-bold hover:bg-amber-700 transition-all shrink-0"
        >
          Xem Tiến Trình
        </button>
      </div>
