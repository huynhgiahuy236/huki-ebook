      ...
      <aside className="absolute inset-y-0 right-0 w-full max-w-2xl bg-white dark:bg-slate-950 shadow-2xl border-l border-slate-200 dark:border-slate-800 flex flex-col animate-slide-in-right">
        ...
        {!loading && order && (
          <footer className="px-5 sm:px-6 py-4 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 flex flex-wrap justify-end gap-2">
            {canCancel && ['PENDING_CONFIRMATION', 'CONFIRMED', 'PREPARING'].includes(order.status) && <button type="button" onClick={() => onCancel(order)} disabled={actionLoading} className="px-4 py-2 rounded-xl border border-rose-200 text-rose-700 font-bold text-sm hover:bg-rose-50 disabled:opacity-50">Hủy đơn hàng</button>}
            {canProcess && order.status === 'PENDING_CONFIRMATION' && <button type="button" onClick={() => onConfirm(order.id)} disabled={actionLoading} className="px-4 py-2 rounded-xl bg-emerald-600 text-white font-bold text-sm disabled:opacity-50">Xác nhận đơn hàng</button>}
            {canProcess && order.status === 'CONFIRMED' && <button type="button" onClick={() => onPrepare(order.id)} disabled={actionLoading} className="px-4 py-2 rounded-xl bg-indigo-600 text-white font-bold text-sm disabled:opacity-50">Chuẩn bị đóng gói</button>}
            {canProcess && order.status === 'PREPARING' && <button type="button" onClick={() => onShip(order)} disabled={actionLoading} className="px-4 py-2 rounded-xl bg-purple-600 text-white font-bold text-sm disabled:opacity-50">Bàn giao vận chuyển</button>}
            {canProcess && order.status === 'SHIPPED' && <button type="button" onClick={() => onDeliver(order.id)} disabled={actionLoading} className="px-4 py-2 rounded-xl bg-teal-600 text-white font-bold text-sm disabled:opacity-50">Xác nhận đã giao</button>}
          </footer>
        )}
      </aside>
    </div>
