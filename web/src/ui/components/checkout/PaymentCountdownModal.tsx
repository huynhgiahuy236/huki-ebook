...
const VIETNAM_BANKS_BIN_MAP: Record<string, string> = {
...
export interface PaymentCountdownModalProps {
  isOpen: boolean;
  onClose: () => void;
  orderId?: string;
  orderCode?: string;
...
export default function PaymentCountdownModal({
      ...
      try {
        ...
        if (payRes.success && payRes.data) {
          const isPaid =
            ...
            payRes.data.payments?.some((p: any) => p.status === 'SUCCEEDED');
          if (isPaid) {
            setIsSuccess(true);
            ...
            setTimeout(() => {
              if (onSuccess) {
                ...
                router.push(`/order-success?orderId=${orderId}&code=${orderCode}&paymentMethod=ONLINE_PAYMENT`);
              }
            }, 1200);
            return;
          }
          ...
          if (o.paymentStatus === 'SUCCEEDED' || o.status === 'PROCESSING' || o.status === 'CONFIRMED' || o.status === 'COMPLETED') {
            ...
            setTimeout(() => {
              if (onSuccess) {
                ...
                router.push(`/order-success?orderId=${orderId}&code=${orderCode || o.code}&paymentMethod=ONLINE_PAYMENT`);
              }
            }, 1200);
  ...
  }, [isOpen, orderId, orderCode, isSuccess, onSuccess, router, showToast]);
