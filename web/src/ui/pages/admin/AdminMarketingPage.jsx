...
export default function AdminMarketingPage() {
  ...
  const handleCreateCampaign = async (e) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        showToast(res.message || "Không thể tạo chiến dịch!", "error");
      }
    } catch (err) {
  ...
  const handleAddItemToSlot = async (e) => {
    e.preventDefault();
    ...
    try {
      ...
      if (res.success) {
        ...
        showToast(res.message || "Không thể thêm sách!", "error");
      }
    } catch (err) {
  ...
  const handleToggleFlashSaleStatus = async (slot) => {
  ...
  const handleDeleteFlashSale = async (id, name) => {
