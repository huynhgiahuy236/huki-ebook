...
export default function AdminBooksPage() {
  ...
  const [suspendModalBook, setSuspendModalBook] = useState(null);
    ...
    try {
      ...
      if (res.success) {
        ...
        showToast({
          ...
          message: res.error || 'Vui lòng kiểm tra lại quyền hạn!',
          type: 'error',
        });
      }
    } catch (err) {
  ...
  const handleActivate = async (book) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        showToast({
          ...
          message: res.error || 'Không thể mở khóa sách.',
          type: 'error',
        });
      }
    } catch (err) {
            ...
            <table className="w-full text-left border-collapse text-xs">
              ...
              <tbody className="divide-y divide-gray-100">
                {filteredBooks.map((book) => {
                  ...
                  const fmt = FORMAT_CONFIG[book.format] || FORMAT_CONFIG.PHYSICAL;
              ...
              <div className="p-4 rounded-xl bg-gray-50 border border-gray-200 flex gap-4">
                ...
                <div className="flex-1 min-w-0 flex flex-col justify-between">
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold border ${(STATUS_CONFIG[selectedBook.status?.toUpperCase()] || STATUS_CONFIG.DRAFT).badge}`}>
                      {(STATUS_CONFIG[selectedBook.status?.toUpperCase()] || STATUS_CONFIG.DRAFT).label}
                    </span>
                    <span className={`px-2 py-0.5 rounded text-[10px] font-bold border ${(FORMAT_CONFIG[selectedBook.format] || FORMAT_CONFIG.PHYSICAL).color}`}>
                      {(FORMAT_CONFIG[selectedBook.format] || FORMAT_CONFIG.PHYSICAL).label}
                    </span>
                  </div>
                </div>
              </div>
