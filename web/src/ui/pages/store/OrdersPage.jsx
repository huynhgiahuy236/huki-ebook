...
export default function OrdersPage() {
  ...
  const filteredOrders = useMemo(() => {
    return orders.filter((ord) => {
      ...
      const matchItems = ord.sellerOrders?.some((so) =>
        so.items?.some((it) => it.bookTitle?.toLowerCase().includes(search))
      );
    ...
    <div className="w-full min-h-screen flex flex-col font-sans text-on-surface bg-theme-bg py-6 md:py-8">
      <div className="max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <nav className="flex items-center gap-2 text-xs font-medium text-on-surface-variant mb-6">
          ...
          <span>/</span>
          <Link to="/profile" className="hover:text-theme-primary transition-colors">
            Tài khoản
          </Link>
          <span>/</span>
        ...
        {isLoading ? (
          ...
          <div className="space-y-4">
            {filteredOrders.map((ord) => {
              ...
              const totalItems =
                ord.sellerOrders?.reduce((sum, so) => sum + (so.items?.length || 0), 0) || 0;
