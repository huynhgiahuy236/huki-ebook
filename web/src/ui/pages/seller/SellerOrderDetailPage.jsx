...
const dateTime = (val) => (val ? new Date(val).toLocaleString('vi-VN') : '—');
...
export default function SellerOrderDetailPage() {
  ...
  const [modalState, setModalState] = useState(null); // { type: 'SHIP' | 'CANCEL' }
  ...
  const hasVatInvoice = Boolean(
    ...
    (order?.id && typeof window !== 'undefined' && localStorage.getItem(`huki_order_vat_${order.id}`) === 'true') ||
    (order?.orderId && typeof window !== 'undefined' && localStorage.getItem(`huki_order_vat_${order.orderId}`) === 'true') ||
    (order?.code && typeof window !== 'undefined' && localStorage.getItem(`huki_order_vat_${order.code}`) === 'true')
  );
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 text-xs text-on-surface-variant">
          <Link to="/seller/dashboard" className="hover:text-primary">Kênh Người Bán</Link>
          <span>/</span>
          <Link to="/seller/orders" className="hover:text-primary">Quản Lý Đơn Hàng</Link>
          <span>/</span>
      ...
      <div className="bg-surface-container-lowest border border-outline-variant rounded-2xl p-5 sm:p-6 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          {hasVatInvoice && (
            <Link
              to={`/orders/${order.orderId || order.id}/invoice`}
              target="_blank"
      ...
      <div className="grid grid-cols-12 gap-6">
        {/* Left Column (8 Cols): Order Items, Notes, VAT & Timeline */}
        <div className="col-span-12 lg:col-span-8 space-y-6">
          {/* 1. Products in Order */}
          <section className="bg-surface-container-lowest border border-outline-variant rounded-2xl overflow-hidden shadow-xs">
            ...
            <div className="divide-y divide-outline-variant/60">
              {(order.items || []).map((item, idx) => {
                ...
                const itCover = item.coverUrl || item.coverImage || item.bookCoverUrl || '/banners/hero-library.jpg';
          ...
          {/* 2. Customer Order Note (Explicit block) */}
          ...
          {/* 3. VAT Invoice Section (Explicit block) */}
        ...
        {/* Right Column (4 Cols): Customer info, Payment, Financial summary & Logistics */}
        <div className="col-span-12 lg:col-span-4 space-y-6">
