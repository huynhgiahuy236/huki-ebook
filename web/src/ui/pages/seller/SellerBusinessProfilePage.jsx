...
export default function SellerBusinessProfilePage() {
  ...
  // Khởi tạo form state từ user context nếu có sẵn
  const initialBiz = user?.business || null;
  ...
  const loadData = useCallback(async (showIndicator = true) => {
    ...
    try {
      ...
      if (bizRes && bizRes.success && bizRes.data) {
        b = bizRes.data?.data || bizRes.data;
      }
      if (!b && user?.business) {
        b = user.business;
      }
      ...
      if (bizId) {
        ...
        if (storesRes && storesRes.success && storesRes.data) {
          const stList = Array.isArray(storesRes.data) ? storesRes.data : storesRes.data?.data || [];
          if (stList.length > 0) {
            setStores(stList);
            const st = stList[0];
            setFormData((prev) => ({
      ...
      if (reqRes && reqRes.success && reqRes.data) {
        ...
        if (reqRes.data.cooldownRemaining) {
          setCooldownSeconds(reqRes.data.cooldownRemaining);
        }

        // Tính số thông báo phản hồi (loại trừ đã xóa và đã đọc)
  ...
  // Lắng nghe sự kiện cập nhật/xóa thông báo để đồng bộ số lượng chấm đỏ ngay lập tức
  useEffect(() => {
  ...
  // Thao tác Trụ sở (Thêm / Xóa / Thay đổi)
  ...
  const handleRemoveHeadquarter = (index) => {
  ...
  const handleHqChange = (index, value) => {
    setFormData((prev) => {
  ...
  const handleSubmitUpdateRequest = async (e) => {
    ...
    try {
      ...
      if (res.success) {
        showToast(res.message || 'Đã gửi yêu cầu cập nhật thông tin thành công!', 'success');
        setIsEditMode(false);
  ...
  const handleOpenNotification = () => {
    navigate('/seller/business/notifications');
  };
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-4 py-8 sm:px-6 lg:px-8 space-y-8 animate-fade-in relative pb-24">
      ...
      <header className="flex flex-col gap-4 border-b border-theme-border pb-6 sm:flex-row sm:items-end sm:justify-between">
        ...
        {/* Action Buttons: [🔔 Chuông thông báo] -> [🔄 Làm mới] -> [✏️ Cập nhật thông tin] */}
      ...
      <form onSubmit={handleSubmitUpdateRequest} className="space-y-8">
              ...
              <div>
                ...
                {isEditMode ? (
                  <input
                    ...
                    className="w-full h-12 px-4 rounded-xl border border-gray-300 dark:border-gray-700 bg-white dark:bg-[#1f1a29] focus:border-theme-primary focus:ring-4 focus:ring-theme-primary/10 transition-all text-sm font-semibold text-on-surface placeholder:text-gray-400 placeholder:font-normal outline-none shadow-xs"
                ...
                <div>
                  ...
                  {isEditMode ? (
                    <input
                      ...
                      className="w-full h-12 px-4 rounded-xl border border-gray-300 dark:border-gray-700 bg-white dark:bg-[#1f1a29] focus:border-theme-primary focus:ring-4 focus:ring-theme-primary/10 transition-all text-sm font-mono font-bold text-on-surface placeholder:text-gray-400 placeholder:font-normal outline-none shadow-xs"
                ...
                <div>
                  ...
                  {isEditMode ? (
                    <input
                      ...
                      className="w-full h-12 px-4 rounded-xl border border-gray-300 dark:border-gray-700 bg-white dark:bg-[#1f1a29] focus:border-theme-primary focus:ring-4 focus:ring-theme-primary/10 transition-all text-sm font-medium text-on-surface placeholder:text-gray-400 placeholder:font-normal outline-none shadow-xs"
        ...
        {/* Card 3: TRỤ SỞ & CHI NHÁNH (CÓ NÚT THÊM TRỤ SỞ 2, 3... ĐẾM TĂNG DẦN) */}
                ...
                <div>
                  {isEditMode ? (
                    <input
                      ...
                      placeholder={`Nhập địa chỉ chi tiết cho Trụ sở ${idx + 1} (Số nhà, Tên đường, Phường/Xã, Quận/Huyện, Tỉnh/Thành phố)...`}
