...
export interface CartContextType {
  ...
  hasEbookItems: boolean;
  isLoading: boolean;
  addItem: (itemOrBook: any, format?: string, quantity?: number) => Promise<void>;
  addToCart: (itemOrBook: any, format?: string, quantity?: number) => Promise<void>;
  removeFromCart: (id: string) => Promise<void>;
  updateQuantity: (id: string, delta: number) => Promise<void>;
  toggleCheckItem: (id: string) => void;
  toggleStoreCheck: (storeId: string, isChecked: boolean) => void;
  toggleAll: (isChecked: boolean) => void;
  clearCart: () => Promise<void>;
  clearUnavailableItems: () => Promise<void>;
...
export const CartProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const persistCart = useCallback(
    (items: CartItem[], targetUser = user) => {
  ...
  const mapServerItemToLocal = useCallback((serverItem: any, existingLocalItems: CartItem[] = []): CartItem => {
  ...
  const fetchServerCart = useCallback(async () => {
    ...
    try {
      ...
      if (res.success && res.data?.items) {
        const mapped = res.data.items.map((serverItem: any) =>
        ...
        try {
          ...
          if (guestItems.length > 0) {
            ...
            if (payloadItems.length > 0) {
              const res = await cartApi.mergeCart(payloadItems);
              if (res.success && res.data?.items) {
                const mapped = res.data.items.map((serverItem: any) => mapServerItemToLocal(serverItem, guestItems));
                setCartItems(mapped);
                persistCart(mapped, user);
  ...
  const addItem = useCallback(
    async (itemOrBook: any, format = 'paper') => {
      ...
      if (isLoggedIn && isUUID) {
        try {
          ...
          if (res.success && res.data?.items) {
            setCartItems((prev) => {
              const updated = res.data!.items.map((srvItem: any) => mapServerItemToLocal(srvItem, prev));
