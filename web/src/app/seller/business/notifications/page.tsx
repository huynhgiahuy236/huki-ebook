...
interface UpdateRequestNotification {
  id: string;
  status?: string;
  createdAt: string;
...
export default function SellerBusinessNotificationsPage() {
  ...
  const [requests, setRequests] = useState<UpdateRequestNotification[]>([]);
  const [activeTab, setActiveTab] = useState<'ALL' | 'APPROVED' | 'REJECTED' | 'PENDING'>('ALL');
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedRequest, setSelectedRequest] = useState<UpdateRequestNotification | null>(null);
  ...
  const markAsRead = (reqId: string) => {
    if (!bizId || readMap[reqId] || typeof window === 'undefined') return;
  ...
  const markAllAsRead = () => {
    if (!bizId || nonDeletedRequests.length === 0 || typeof window === 'undefined') return;
  ...
  // Xóa 1 thông báo
  const handleDeleteOne = (reqId: string, e?: React.MouseEvent) => {
    e?.stopPropagation?.();
    if (!bizId || typeof window === 'undefined') return;
  ...
  const handleBulkDelete = () => {
    if (!bizId || selectedIds.length === 0 || typeof window === 'undefined') return;
  ...
  const handleToggleSelect = (reqId: string, e?: React.MouseEvent) => {
    e?.stopPropagation?.();
    setSelectedIds((prev) =>
  ...
  const handleCardClick = (req: UpdateRequestNotification) => {
    markAsRead(req.id);
    ...
    <div className="mx-auto w-full max-w-6xl px-4 py-8 sm:px-6 lg:px-8 space-y-8 animate-fade-in pb-24">
      ...
      <header className="space-y-4 border-b border-theme-border pb-6">
        <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-theme-primary">
          ...
          <span>&bull;</span>
          <Link href="/seller/business" className="hover:underline">
            Hồ Sơ Doanh Nghiệp
          </Link>
          <span>&bull;</span>
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-gray-100 dark:border-gray-800">
                  <div className="flex items-center gap-3">
                    <input
                      ...
                      onChange={(e) => handleToggleSelect(req.id, e as any)}
                      className="w-4 h-4 rounded text-theme-primary border-gray-300 focus:ring-theme-primary cursor-pointer accent-theme-primary"
                    />
