...
export default function FlashSalePage() {
  ...
  useEffect(() => {
    const tick = () => {
      ...
      setActiveSlotId((current) => {
        const previousMode = current ? previousModes.current[current] : undefined;
        if (
          current &&
          previousMode === "active" &&
  ...
  const displayItems: DisplayFlashItem[] = useMemo(() => {
  ...
  const handleRemindMe = (slotName: string, itemTitle: string) => {
    showToast(
