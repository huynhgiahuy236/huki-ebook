...
export default function AuthorPage() {
  ...
  const copyQuote = (quoteText: string) => {
  ...
  return (
    <div className="w-full max-w-[1520px] mx-auto px-4 sm:px-6 lg:px-8 py-6 pb-20 font-body-md">
      ...
      <nav className="text-body-sm text-on-surface-variant mb-6 flex items-center gap-2 flex-wrap">
        <Link href="/" className="hover:text-[#ac2c19] transition-colors">Trang chủ</Link>
        <span>/</span>
        <Link href="/books" className="hover:text-[#ac2c19] transition-colors">Sàn TMĐT Sách</Link>
        <span>/</span>
        <span className="text-theme-primary font-semibold">Tác Giả {id === 'james-clear' ? 'James Clear' : id}</span>
      </nav>
        ...
        <div className="lg:col-span-7 bg-theme-surface rounded-3xl border border-theme-border p-6 sm:p-8 shadow-sm flex flex-col justify-between">
          <div>
            ...
            <div className="space-y-4 text-sm sm:text-base text-on-surface leading-relaxed">
              <p>
                James Clear là nhà văn và diễn giả nổi tiếng tập trung vào các chủ đề thói quen, ra quyết định và cải thiện liên tục. Cuốn sách <strong>&ldquo;Atomic Habits&rdquo; (Thói Quen Nguyên Tử)</strong> của ông đã bán được hơn 15 triệu bản trên toàn cầu, được dịch ra hơn 50 ngôn ngữ và trở thành một trong những cẩm nang phát triển bản thân có sức ảnh hưởng sâu rộng nhất thế kỷ 21.
              </p>
              <p>
          ...
          <div className="mt-8 pt-6 border-t border-theme-border bg-theme-secondary-subtle -mx-6 sm:-mx-8 -mb-6 sm:-mb-8 p-6 sm:p-8 rounded-b-3xl flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <Link href="/library/devices" className="bg-theme-secondary hover:opacity-90 text-white px-4 py-2 rounded-xl text-xs font-semibold whitespace-nowrap shadow-xs transition-colors">
              Quản Lý Thiết Bị
            </Link>
          </div>
        </div>

        {/* Right (5/12): Quote Cards */}
          ...
          {filteredWorks.map(work => (
            ...
            >
              <div>
                ...
                <Link href={`/book/${work.id}`} title={work.title}>
                  ...
                  <button
                    ...
                    onClick={() => {
                      addToCart(
                        ...
                        work.format === 'physical' ? 'paper' : 'ebook',
                        1,
                      );
