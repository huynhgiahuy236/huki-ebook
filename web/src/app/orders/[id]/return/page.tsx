...
export default function OrderReturnPage() {
  ...
  const id = rawId || 'HUKI-8892401';
  const { showToast } = useToast();
  const router = useRouter();

  const [selectedItems, setSelectedItems] = useState<Record<string, boolean>>({ 'atomic-habits': true });
  const [quantities, setQuantities] = useState<Record<string, number>>({ 'atomic-habits': 1, 'psychology-money': 1 });
  ...
  return (
    <div className="w-full max-w-[1240px] mx-auto px-4 sm:px-6 lg:px-8 py-6 pb-20 font-body-md">
      ...
      <nav className="text-body-sm text-on-surface-variant flex items-center gap-2 mb-6">
        ...
        <span>/</span>
        <Link href="/orders" className="hover:text-primary transition-colors">Lịch sử đơn hàng</Link>
        <span>/</span>
        <Link href={`/orders/${id}`} className="hover:text-primary transition-colors">Đơn hàng #{id}</Link>
        <span>/</span>
