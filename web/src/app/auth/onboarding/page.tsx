...
export default function OnboardingPreferencesPage() {
  ...
  const [selectedGenres, setSelectedGenres] = useState<string[]>(['self-dev', 'business', 'tech-ai']);
  ...
  const handleFinish = () => {
    if (selectedGenres.length === 0) {
      showToast({
        ...
        message: 'Vui lòng chọn ít nhất 1 thể loại sách yêu thích!',
        type: 'warning',
      });
      return;
    }
    ...
    showToast({
      ...
      message: 'Đã lưu sở thích đọc sách & kích hoạt gói quà chào mừng 50.000đ!',
      type: 'success',
    });
    setTimeout(() => {
      router.push('/');
        ...
        <div className="bg-white rounded-3xl border border-[#e8e5df] p-6 sm:p-8 shadow-sm space-y-6">
          ...
          <div className="pt-4 border-t border-[#e8e5df] flex flex-col sm:flex-row items-center justify-between gap-3">
            <button
              type="button"
              onClick={() => router.push('/')}
              className="text-xs font-semibold text-[#6b7280] hover:text-[#17201f] py-2 px-3 order-2 sm:order-1 cursor-pointer"
            >
