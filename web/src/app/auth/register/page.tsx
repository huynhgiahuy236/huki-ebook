...
export default function RegisterPage() {
  ...
  const validateSingleField = (field: string, value: any, allData = formData) => {
    try {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (regRes.success) {
      ...
      router.push(`/auth/verify?email=${encodeURIComponent(formData.email.trim())}`);
    } else {
      ...
      <div className="grid min-h-dvh w-full grid-cols-1 overflow-hidden bg-white lg:grid-cols-12">
        ...
        <section className="flex min-h-dvh flex-col justify-between bg-white px-6 py-8 sm:px-12 sm:py-10 lg:col-span-7 lg:px-14 xl:px-20 2xl:px-28">
            ...
            <form onSubmit={handleSubmit} noValidate className="space-y-4">
              ...
              <div className="pt-1.5">
                <label className="flex items-start gap-2.5 cursor-pointer text-xs text-slate-600 select-none">
                  ...
                  <span>
                    Tôi đồng ý với{' '}
                    <Link href="/terms" className="text-slate-900 font-bold underline hover:text-[#003b2b]">
                      Điều khoản dịch vụ
                    </Link>{' '}
                    và{' '}
                    <Link href="/privacy" className="text-slate-900 font-bold underline hover:text-[#003b2b]">
                      Chính sách bảo mật
                    </Link>{' '}
                    của HUKI.
                  </span>
          ...
          <div className="mt-8 pt-4 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500">
            <div>
              Đã có tài khoản?{' '}
              <Link href="/auth/login" className="font-bold text-[#003b2b] hover:underline">
                Đăng nhập ngay
              </Link>
            </div>
            <Link
