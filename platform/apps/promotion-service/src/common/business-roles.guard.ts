...
export function BusinessRoles(...roles: string[]) {
  return (
    target: any,
    propertyKey?: string,
...
export class BusinessRolesGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    // Try to decode JWT if not already set
    if (!user) {
    ...
    const hasRole = requiredRoles.some((role) => {
      // OWNER can access anything business-related
    ...
    if (!hasRole) {
      throwForbidden(
        ...
        "Insufficient permissions. Business owner role required.",
      );
      return false;
    }
